...
*** 25aug00 version
*** triple difference carrier phase on l1
*** WARNING: not precisely identical to DD1   (dgm 29-oct-95)
*** version (0.3  test initiated 11-jan-95)
...
*** note: input file names currently hard-coded
      ...
      lst=1
      open(lst,file='tdxl.lst',form='formatted')
      write(lst,*) 'Triple Difference -- L1 Program -- 2005sep17'
...
*** get antenna coordinates -- if needed, borrow the lpos logical unit
...
*** apply corrections one time only in formob()
     ...
     *            icols1,ndat1,icols2,ndat2,rms,xmax)

*** loop over the adjustment  (do auto-rejection)
*** converge if max shift at 5 mm and rms stable to 5%
...
10000 if(iter.le.maxitr) then
        ...
        rejlvl=siglvl*rms
        call comrms(orbits,coord,rejlvl,rms,xmax,nsat)
        iter=iter+1
        if(dabs(xmax).gt.postol.or.(dabs(rms-rmsx)/rms).gt.sigtol) then
          xmaxx=xmax
          rmsx =rms
          go to 10000
        endif
...
*** provide results
      ...
      lres=9
      open(lres,file='tdxl.res',form='formatted')
...
*** loop over obs. equations
      ...
      rewind(lobs)
  100 read(lobs,end=777) iisat,trec1,trec2,r110,r210,r11,r21,pdd1

*** header record -- read another right away to init. trip. diffs.

      if(iisat.le.0) then
        read(lobs,end=777) iisat,trec1,trec2,r110,r210,r11,r21,pdd1
        if(iisat.gt.nsat) go to 100
        if(.not.getrs(iirfsv,trec1)) stop 14379
...
*** trailer record -- no action

      elseif(iisat.gt.nsat) then
        continue

*** form triple differences --   sr,s geometric range, cmo is d3-phase

      else
        if(.not.getrs(iirfsv,trec1)) stop 25397
        call comdtr(iirfsv,coord,orbits,trec1,trec2,r110,r210,dtr1,dtr2)
        call comrng(iirfsv,coord,orbits,trec1,trec2,r110,r210,dtr1,dtr2,
     *              sr1,sr2,cdum)
     ...
     *              s1,s2,gla,glo,vad)
        ...
        write(lres,'(f9.1,f15.5,f6.2,i3)') trec1,cmo,vad,isat
        if(dabs(cmo).le.rejlvl) then
          nsv(isat)=nsv(isat)+1
          rmss(isat)=rmss(isat)+cmo*cmo
        endif

*** save current for the next triple difference
  ...
  777 rewind(lobs)
      ...
      do 10 i=1,maxsat
        if(nsv(i).gt.0) then
          ...
          write(lst,'(i2,i8,2f20.4)') i,nsv(i),rmss(i),rms2(i)
...
*** deleted satellite ??
        ...
        elseif(nsv2(i).gt.0) then
          rmss(i)=0.d0
          rms2(i)=dsqrt(rms2(i)/dble(nsv2(i)))
          write(lst,'(i2,i8,2f20.4,a)') i,nsv(i),rmss(i),rms2(i),' ****'
        endif
   10 continue
      rms=dsqrt(rms/dble(iobs))
      write(lst,'(a,f9.5,a,i6)') 'post-fit rms =',rms,' m.    n=',iobs
...
************************************************************************
      subroutine comrms(orbits,coord,rejlvl,rms,xmax,nsat)

*** routine to evaluate obs. equations, form and solve normals

      implicit double precision(a-h,o-z)
      logical getrs
      dimension orbits(*),coord(20),c(3),cr(3),cd(3),cx(3)
...
*** header record -- read another right away to init. trip. diffs.
      ...
      if(iisat.le.0) then
        read(lobs,end=777) iisat,trec1,trec2,r110,r210,r11,r21,pdd1
        if(iisat.gt.nsat) go to 100
        if(.not.getrs(iirfsv,trec1)) stop 44379
        call comdtr(iirfsv,coord,orbits,trec1,trec2,r110,r210,dtr1,dtr2)
        call comrng(iirfsv,coord,orbits,trec1,trec2,r110,r210,dtr1,dtr2,
     *              sr1,sr2,cr)
        call comrng(iisat ,coord,orbits,trec1,trec2,r11,r21,dtr1,dtr2,
...
*** form triple differences --   sr,s geometric range, cmo is d3-phase
        ...
        if(.not.getrs(iirfsv,trec1)) stop 45397
        call comdtr(iirfsv,coord,orbits,trec1,trec2,r110,r210,dtr1,dtr2)
        call comrng(iirfsv,coord,orbits,trec1,trec2,r110,r210,dtr1,dtr2,
     *              sr1,sr2,cr)
        call comrng(iisat ,coord,orbits,trec1,trec2,r11,r21,dtr1,dtr2,
   ...
   10   cd(i)=(c(i)-cr(i))
        ...
        cmo=cmod-cmox
***** write(lst,'(f9.1,f14.4,i3)') trec1,cmo,invprn(iisat)
        if(dabs(cmo).le.rejlvl) then
          do 11 i=1,3
   11     c(i)=cd(i)-cx(i)
          iobs=iobs+1
...
*** compute rms from "basement window"  (v'pv=l'pl-x'u)
      ...
      write(lst,'(a,i7,a,f9.5,a,f14.6,a)') 
     *  'n =',iobs,'    rms =',rms,' m     max shift = ',xmax,' m'
...
*** form observation equations for the first time
...
*** reference sat
     ...
     *                  dtr1,dtr2,s10,s20,c0,gla1,glo1,eht1,
     *                  gla2,glo2,eht2,tdly10,tdly20)
            ssd0=s20-s10
*** correct observed phase (phase/range measured too long due to tropo)
            tdlsd0=tdly20-tdly10
            psd10=psd10-tdlsd0/w1
...
***  bypass the reference satellite
     ...
     *                        dtr1,dtr2,s1,s2,cd,gla1,glo1,eht1,
...
*** outer loop over all satellites and files
...
*** first record in double difference stream

        read (10+iisat,end=102) trec1,trec2,r110,r210,r11,r21,
     *                          pdd1,sdd1,cx
        write(lobs) iisat,trec1,trec2,r110,r210,r11,r21,pdd1
        cmox=sdd1-pdd1*w1

*** remaining records in double difference stream, reject using tdtol
...
*** retain for next triple difference
...
*** end of file encountered -- write trailer
...
*** insure data intervals are compatible with the obs interval
      ...
      write(lst,'(a,3f10.0,f10.2,a)') 'common start/stop, intrvl',
     *                      start,stop,delt,(stop-start)/3600.d0,' hrs'
...
*** iprns() is the buffer to hold one record of prn numbers
  ...
  109   read(lrin,1) card
      ...
      if(nprn.le.0.or.nprn.gt.mxsat) then
        write(*,*) 'iyr,imo,idy,ihr,imn,sec',iyr,imo,idy,ihr,imn,sec
        write(*,*) 'nprn,mxsat',nprn,mxsat
        stop 24308
      endif
...
*** sanity checks
      ...
      if(gla.lt. -1.571d0 .or. gla.gt. 1.571d0) stop 80004
      if(oht.lt.  -150.d0 .or. oht.gt. 5000.d0) stop 80005
      if(v  .lt.     0.d0 .or. v  .gt. 1.571d0) stop 80006
